...
class ResourceInteractorMock1: ResourceInteracting {
      func fetchResource(_ id: String, completion: @escaping (Result<[Resource]>) -> Void) {
          do {
              if let bundlePath = Bundle.main.path(forResource: "restaurants1", ofType: "json"), let jsonData = try String(contentsOfFile: bundlePath).data(using: .utf8) {
